...
// $Id: RF12demo.pde 5287 2010-04-07 19:03:49Z jcw $
 ...
 */
int unsignedShortToInt(byte MSB,byte LSB) 
{
    int i = 0;
    i |= LSB & 0xFF;
    i <<= 8;
...
static void df_erase (word block) {
    if(DEBUG) Serial.print("DF E ");
    if(DEBUG) Serial.println(block);
...
void loop() {
    ...
    if (rf12_recvDone()) {
        ...
        addr=rf12_hdr & RF12_HDR_MASK;
        //EMMA is calling
          switch(addr)
          {
            case 2:
              emma.type=rf12_data[0];
              emma.value=rf12_data[1];
              emma.lobat=rf12_data[2];
              //then we close the data
              if(emma.type==ALARM)
              {
                if(emma.value==START)
                  Serial.println("ALARM1");
                else if(emma.value==FALL)
                  Serial.println("FALL");
                else 
                  Serial.println("ALARM2");
                
              } else if(emma.lobat==1)
                Serial.println("LBAT");
            break;
            default:
                room.light=rf12_data[0];
                room.moved=rf12_data[1]& 1;
                room.humi=rf12_data[1] >>1;
                byte msb_temp=rf12_data[2];
                byte lsb_temp=rf12_data[3]>>6;
                room.temp=((256 * (rf12_data[3]&3) + rf12_data[2]) ^ 512) - 512;
                room.lobat=(rf12_data[3]>>2) & 0x01;
                //we ignore the movement sensors because my HW doesn't work
                Serial.print("L:");
                Serial.print(room.light,HEX);
                Serial.print("H:");
                Serial.print(room.humi,HEX);
                Serial.print("T:");
                Serial.print(room.temp,HEX);
                //Serial.print("P:");
                //Serial.print(room.moved,HEX);
                if(room.lobat==1)
                Serial.print("LBAT");
    ...
    if (cmd && rf12_canSend()) {
        ...
        Serial.println(" b");
        cmd = cmd == 'a' ? RF12_HDR_ACK : 0;
        if (dest)
            cmd |= RF12_HDR_DST | dest;
        rf12_sendStart(cmd, testbuf, sendLen);
        cmd = 0;
